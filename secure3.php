...
if (isset($_POST['delete_videos'])) {
    if (isset($_POST['videos']) && !empty($_POST['videos'])) {
        ...
        echo "<p style='background-color: #d4edda; color: #155724;  padding: 10px; border: 1px solid #c3e6cb; border-radius: 5px;'>Videos and associated data deleted.</p>";
    } else {
...
<a href="Main.php" style="float: right;background-color:#E50914;
			color:white;
            float: right;
            margin-left: 10px;
			 padding: 3px 8px;
			 border-radius:4px;
			 Text-Decoration: None !important;" class="logout">Back</a>
    <div class="container">
...
<br>
                <label  for="view-option"><b>View Videos:</b></label>
				<div class="email-form">
				<select name="view-option" id="view-option" class="email-input">
                        <option value="all">All Videos</option>
                        <option value="uploaded-by-me">Uploaded by Me</option>
                    </select>
                <button class="email-button">View</button>
            </div>
                    ...
                    <form action="" method="POST">
                        ...
                        if ($result && mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo "<div><input type='checkbox' name='videos[]' value='" . $row['id'] . "'><label style='margin-left:10px;colro:white' for='a'><a style='color:white' href='view_video.php?id=" . $row['id'] . "' class='video-link'>" . $row['title'] . "</a></label></div>";
                            }
                        } else {
